...
function renderAWOL() {
  ...
  const groups = [
    ...
    {
      key: "fabricated",
      students: filtered.filter(s => (s.file_type_counts?.unknown || 0) > 0)
                        .sort((a, b) => (b.file_type_counts?.unknown || 0) - (a.file_type_counts?.unknown || 0)),
      badge: "⛔",
      color: "var(--danger)",
      border: "rgba(255,80,80,.25)",
      label: isFr
        ? "Fichiers non conformes (schéma non reconnu — probable création manuelle ou par IA)"
        : "Non-compliant files (unrecognized schema — likely manually or AI-generated)",
    },
        ...
        <div style="text-align:right;flex-shrink:0">
          <div style="color:${color};font-weight:700;font-size:1.5rem">
            ${groupKey === "wraps"
              ...
              : groupKey === "fabricated"
              ? `${s.file_type_counts?.unknown || 0} ${isFr ? "fichier·s non conforme·s" : "non-compliant file·s"}`
