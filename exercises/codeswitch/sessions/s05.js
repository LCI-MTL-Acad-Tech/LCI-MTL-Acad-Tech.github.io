...
<span class="kw2">public</span>:
  ...
  homework:{
    core:[
      {diff:'easy', fr:'Crée un fichier mathutils.h avec #pragma once et une fonction inline int square(int n). Inclus-le dans deux fichiers .cpp différents. Compile et vérifie qu\'il n\'y a pas de doublon.', en:'Create a mathutils.h file with #pragma once and an inline int square(int n) function. Include it in two different .cpp files. Compile and verify there are no duplicates.'},
      {diff:'medium', fr:'Sépare une classe Vector2 en vector2.h (déclaration) et vector2.cpp (implémentation). Compile avec g++ main.cpp vector2.cpp -o main. Pourquoi faut-il lister les deux .cpp ?', en:'Separate a Vector2 class into vector2.h (declaration) and vector2.cpp (implementation). Compile with g++ main.cpp vector2.cpp -o main. Why do both .cpp files need to be listed?'},
      {diff:'hard', fr:'Crée une erreur \'undefined reference\' volontairement (déclare une fonction dans .h, oublie de l\'implémenter dans .cpp). Lis le message du linker et explique-le dans un commentaire.', en:'Intentionally create an \'undefined reference\' error (declare a function in .h, forget to implement it in .cpp). Read the linker message and explain it in a comment.'},
    ],
    ide:[
      {diff:'medium', fr:'Configure une tâche de build VS Code (tasks.json) pour compiler automatiquement avec Ctrl+Shift+B. La commande doit lister tous tes .cpp.', en:'Configure a VS Code build task (tasks.json) to compile automatically with Ctrl+Shift+B. The command should list all your .cpp files.'},
    ],
    engine:[
      {diff:'medium', fr:'Dans Unreal, ouvre le .h et le .cpp générés par un nouveau C++ Actor. Identifie GENERATED_BODY(), #pragma once, et les includes automatiques. Pourquoi sont-ils là ?', en:'In Unreal, open the .h and .cpp generated by a new C++ Actor. Identify GENERATED_BODY(), #pragma once, and the automatic includes. Why are they there?'},
    ],
  },
};
